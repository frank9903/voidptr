...
void Board::init(int width, int height, int level, string seqfile, bool textonly, int seed, bool darkMode, int hiscore) {
    this->width = width;
    this->height = height;
    setlevel(level);
    ...
    if (!textonly) {
        gd = make_shared<GraphicsDisplay>(18, 11, darkMode);
    }
...
void Board::replaceCurBlock(char c) {
    ...
    Block b {BlockType::I, bornLevel, top, left};
    switch(c) {
        case 'I': b = Block {BlockType::I, bornLevel, top, left}; break;
        case 'J': b = Block {BlockType::J, bornLevel, top, left}; break;
        case 'L': b = Block {BlockType::L, bornLevel, top, left}; break;
        case 'O': b = Block {BlockType::O, bornLevel, top, left}; break;
        case 'S': b = Block {BlockType::S, bornLevel, top, left}; break;
        case 'Z': b = Block {BlockType::Z, bornLevel, top, left}; break;
        case 'T': b = Block {BlockType::T, bornLevel, top, left}; break;
    }
    if (isValidMove(b.getCellsInfo())) {
        blocks.pop_back();
